...
pub enum EmbeddedFileResponse {
    Some(EmbeddedFile),
    None,
}

impl From<EmbeddedFile> for EmbeddedFileResponse {
    fn from(embedded_file: EmbeddedFile) -> Self {
        EmbeddedFileResponse::Some(embedded_file)
    }
}

impl From<Option<EmbeddedFile>> for EmbeddedFileResponse {
    fn from(embedded_file: Option<EmbeddedFile>) -> Self {
        match embedded_file {
            Some(embedded_file) => EmbeddedFileResponse::Some(embedded_file),
            None => EmbeddedFileResponse::None,
        }
    }
}
    ...
    fn respond_to(self, req: &HttpRequest) -> HttpResponse<Self::Body> {
        match self {
            EmbeddedFileResponse::Some(embedded_file) => {
                ...
                let etag = format!(
                    "\"{}\"",
                    base64::encode(embedded_file.metadata.sha256_hash())
                );

                let last_modified_date = embedded_file
                    .metadata
                    .last_modified()
                    ...
                    .and_then(parse_if_none_match_value)
                {
                    if req_etags.contains(&etag.as_str()) {
                        ...
                        return respond(req, embedded_file, &etag, Some(&last_modified));
                    }
                }
                ...
                // send a response.
                respond(req, embedded_file, etag.as_str(), Some(&last_modified))
            }
            EmbeddedFileResponse::None => HttpResponse::NotFound().finish(),
        }
    }
}
...
fn respond(
    ...
    let file_data = embedded_file.data.clone().into_owned();
